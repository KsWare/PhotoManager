...
namespace KsWare.PhotoManager.Helper
{
	...
	public class ShellContextMenu : NativeWindow
	{
		...
		protected IntPtr[] GetPIDLs(IList<FileInfo> arrFI)
		{
			if (null == arrFI || 0 == arrFI.Count)
			{
				return null;
			}
			...
			var arrPIDLs = new IntPtr[arrFI.Count];
			var n = 0;
		...
		public void ShowContextMenu(IList<FileInfo> files, System.Windows.Point pointScreen)
		{
			// Release all resources first.
			ReleaseAll();
		...
		{
			var pointScreenDrawing=new Point(Convert.ToInt32(pointScreen.X), Convert.ToInt32(pointScreen.Y));
			IntPtr pMenu = IntPtr.Zero,
				iContextMenuPtr = IntPtr.Zero,
			...
			try
			{
				...
				var nSelected = TrackPopupMenuEx(
					pMenu,
					TPM.RETURNCMD,
					pointScreenDrawing.X,
					pointScreenDrawing.Y,
					Handle,
					IntPtr.Zero);
				...
				if (nSelected != 0)
				{
					InvokeCommand(_oContextMenu, nSelected, _strParentFolder, pointScreenDrawing);
				}
			}
			catch
			{
